...
pub fn mem_value(item: TokenStream) -> TokenStream {
    ...
    let output = if add_set_function {
        quote::quote!{
            fn #address_func_name(process_memory: &vmemory::ProcessMemory, config: &crate::config::AppConfig) -> usize {
                crate::config::parse_offsets_from_ce_string(&config.addresses.#variable_name).fetch_address(process_memory)
            }
    
            fn #get_func_name(process: &vmemory::ProcessMemory, config: &crate::config::AppConfig) -> #value_type {
                let address = #address_func_name(process, config);
            ...
            fn #set_func_name(process: &vmemory::ProcessMemory, config: &crate::config::AppConfig, value: #value_type) {
                let address = #address_func_name(process, config);
            ...
            async fn #http_get_func_name(process_extension: axum::Extension<std::sync::Arc<self::UndertaleGame>>) -> impl axum::response::IntoResponse {
                ...
                let #variable_name = #get_func_name(&process, &process_extension.config);
            ...
            async fn #http_set_func_name(
                ...
                let process = process_extension.process.lock().await;
                #set_func_name(&process, &process_extension.config, body.#variable_name);
